...
public class EditorDropTarget implements DropTargetListener {
	...
	public EditorDropTarget(JEditorPane pane) {
	  ...
	  protected boolean dropFile(Transferable transferable) throws IOException,
	    ...
	    try {
	    	String uri = null;
	    //	if (pane instanceof MxroEditorPane) {
	    //		MxroEditorPane mxropane = (MxroEditorPane) pane;
	    //		uri = mxropane.handleFile(transferFile);
	    //	} else {
	    //		uri = transferFile.toURI().toString();
	    //	}
            if (pane instanceof MxroTextPane) {
	    		MxroTextPane mxropane = (MxroTextPane) pane;
	    		uri = mxropane.handleFile(transferFile);
	    	} else {
	    		uri = transferFile.toURI().toString();
	    	}
	    	...
	    	if (extension.equals("png") || extension.equals("jpg") || extension.equals("gif") ||
	    		...
	    		((HTMLEditorKit) pane.getEditorKit()).insertHTML((HTMLDocument) pane.getDocument(), pane.getCaretPosition(), "<a href='"+uri +"'>"+transferFile.getName()+"</a>", 0, 0, HTML.Tag.A);
	    	}
      ...
      protected MxroTextPaneFactory ekit;
